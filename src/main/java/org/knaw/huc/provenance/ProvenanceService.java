...
import static org.knaw.huc.provenance.Config.JDBI;

public class ProvenanceService {
    private static final String EXISTS_SQL = "SELECT id FROM provenance WHERE id = :id";

    private static final String INSERT_SQL = "INSERT INTO provenance (" +
    ...
    private static final String UPDATE_SQL = "UPDATE provenance SET " +
    ...
    public int createRecord(ProvenanceInput provenanceInput) {
        try (Handle handle = JDBI.open()) {
            int id = handle.createUpdate(INSERT_SQL)
                    .bind("who", provenanceInput.getWho())
                    .bind("where", provenanceInput.getWhere())
                    .bind("when", provenanceInput.getWhen())
                    .bind("how_software", provenanceInput.getHow())
                    .bind("how_delta", new NullArgument(CHAR))
                    .bind("why_motivation", provenanceInput.getWhy())
                    .bind("why_prov", new NullArgument(CHAR))
                    .executeAndReturnGeneratedKeys()
                    .mapTo(Integer.class)
                    .one();

            insertResources(handle, INSERT_SOURCE_SQL, id, provenanceInput.getSource());
            insertResources(handle, INSERT_TARGET_SQL, id, provenanceInput.getTarget());
    ...
    public void updateRecord(int id, ProvenanceInput provenanceInput) {
        try (Handle handle = JDBI.open()) {
            ...
            insertResources(handle, UPSERT_SOURCE_SQL, id, provenanceInput.getSource());
            insertResources(handle, UPSERT_TARGET_SQL, id, provenanceInput.getTarget());
        }
    }
    ...
    private static void insertResources(Handle handle, String sql,
        ...
        resourceInputs.forEach(target -> batch
                .bind("prov_id", id)
                .bind("res", target.getResource())
                .bind("rel", target.getRelation())
                .add());
        batch.execute();
    }
}
