...
export async function provenanceLoader({ params }: { params: any }) {
    const resource = params.resource;
    const urlParams = new URLSearchParams();

    urlParams.append('resource', resource);
    urlParams.append('direction', 'backwards');

    const result = await fetch(`/trail?${urlParams.toString()}`);
    if (!result.ok)
        throw new Error(`Failed to load trail trail`);

    const trail = await result.json();

    return {trail} as {trail: IResource}
}


export function Provenance() {

    const {trail} = useLoaderData() as {trail: IResource}

    return (
        <>
            <div className="">
                <div className="bg-neutral-100 flex flex-row md:flex-col px-4  rounded py-6">
                    <NavLink to={`/` + encodeURIComponent(trail.resource)} className="whitespace-nowrap no-underline text-left">Data trail</NavLink>
                    <NavLink to={`/` + encodeURIComponent(trail.resource) + `/flow`} className="whitespace-nowrap no-underline text-left">Flow</NavLink>
                </div>
            </div>
